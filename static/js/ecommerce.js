$(document).ready(function(){
    ...
    function displaySearching(){
      ...
      searchBtn.html("<i class='fa fa-spin fas fa-spinner'></i>")
    }
  ...
  productForm.submit(function(event){
      ...
      $.ajax({
        ...
        success: function(data){
          ...
          if (data.added){
            submitSpan.html("<button type='submit' class='btn btn-warning btn-sm'>Remove?</button>")
          } else {
            submitSpan.html("<button type='submit'  class='btn btn-primary btn-sm'>Add to cart</button>")
           }
        ...
        if (data.products.length > 0){
            ...
            $.each(data.products, function(index, value){
              ...
              newCartItemRemove.find(".cart-item-product-id").val(value.id)
                cartBody.prepend("<tr><th scope=\"row\">" + i + "</th><td><a href='" + value.url + "'>" + value.name + "</a>" + newCartItemRemove.html() + "</td><td>" + value.price + "</td><td>" + value.image + "</td></tr>")
                i --
            })
